...
local VScreens_OptionsFrame = CreateFrame("Frame", nil, InterfaceOptionsFramePanelContainer)-- 5
...
VScreens_OptionsFrame:SetScript("OnShow", function(self)
    ...
    guangao:SetPoint("RIGHT")
    guangao:SetText("作者：卢瑟#51517")
  local counta = 0
...
function frameEvent(event,message, sender, flags, channelName,guid)
	if VScreens_Options_v == nil or VScreens_Options == nil then return end -- 3
	if VScreens_Options[event] == false then return end -- 4
...
function frame:ADDON_LOADED(event,...)
	...
	if VScreens_Options_v == nil then VScreens_Options_v = {} end -- 2
...
1.VScreens.toc添加一行## SavedVariables: VScreens_Options,VScreens_Options_v   意思是用这两个变量存放配置到本地
2.在frame:ADDON_LOADED 里初始化 VScreens_Options 和 VScreens_Options_v 变量，并设置初始值
3.if VScreens_Options_v == nil or VScreens_Options == nil then return end  防止消息加载过快发生的错误
4.if VScreens_Options[event] == false then return end   过滤语句
5.添加设置界面
--]]
